        ...
        <div class="modal fade" id="kt_modal_4" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    ...
                    <form id="main_category_form" method="post"
                          @submit.prevent="!editMode ?save() : update(main_category_form.id)">
                        <input type="hidden" name="id" :value="main_category_form.id">
                        <div class="modal-body">
                            <div class="form-group">
                                <label class="form-control-label">الاسم:</label>
                                <input v-model="main_category_form.name" type="text"
                            ...
                            <div class="form-group">
                                ...
                                <div class="card card-custom">
                                    <textarea v-model="main_category_form.description" name="description"
                                              type="text"></textarea>
                                </div>
            ...
            <div class="kt-portlet kt-portlet--mobile h-100">
                    ...
                    <div class="kt-portlet__head-toolbar">
                        <div class="kt-portlet__head-wrapper">
                            <div class="kt-portlet__head-actions">
                                ...
                                <button v-show="this.CheckPremission('main_categories_create')" type="button" class="btn btn-brand btn-elevate btn-icon-sm"
                ...
                <div class="kt-portlet__body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered products-table">
                            ...
                            <tr class="text-center">
                                ...
                                <th class="font-weight-bold "> عدد الاقسام الفرعية</th>
                            ...
                            <tbody>
                            <tr class="text-center" v-for="main_category in main_categories.data">
                                <td>{{ main_category.name }}</td>
                                <td v-if="main_category.description">{{ main_category.description }}</td>
                                <td v-else="" style="color: red">غير مدخل</td>
                                <td >{{ main_category.categories_count }}</td>
                                <td>{{ main_category.created_by }}</td>
                                <td v-if="main_category.updated_by ">{{ main_category.updated_by }}</td>
                                <td v-else >لم يتم التعديل</td>
                                <td class="text-success font-weight-bold" v-if="main_category.status ==1">
                                    مفعل
                                </td>
                                ...
                                <td>
                                    <button v-show="CheckPremission('main_categories_update')"  class="btn btn-primary btn-sm" @click="edit(main_category)"><i
                                        class="fa fa-edit"></i></button>
                                    <button v-show="CheckPremission('main_categories_delete')"  class="btn btn-danger btn-sm" @click="remove(main_category.id)"><i
                                        class="fa fa-trash-alt"></i></button>
                                    <button title="تعطيل" v-if="main_category.status ==1"
                                            class="btn btn-warning btn-sm"
                                            @click="disable(main_category.id)"><i
                                        class="fa fa-window-close"></i></button>
                                    <button title="تفعيل" v-else class="btn btn-success btn-sm"
                                            @click="active(main_category.id)"><i
                    ...
                    <div class="d-flex justify-content-end mt-3">
                        <pagination :data="main_categories" @pagination-change-page="load">
...
export default {
    ...
    methods: {
        load(page = 1) {
            axios.get('get-main-categories?page=' + page, {
                params: {
        ...
        save() {
            ...
            axios.post('/main-categories', formData
                , {})
                .then(response => {
        ...
        remove(id) {
            swal.fire({
                ...
                if (result.value) {
                    axios.delete('/main-categories/' + id)
                        .then((response) => {
                            this.load();
